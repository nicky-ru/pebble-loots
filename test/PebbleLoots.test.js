...
describe('Pebble Loots', () => {
  ...
  before('get factories and init registration', async () => {
    ...
    await this.registration.connect(admin).grant(regOperator.address);

    await this.registration.connect(regOperator).ship([imei1], [device1.address]);

    await this.registration.connect(regOperator).setOwner(imei1, deviceOwner1.address);
  });
  ...
  describe('updates', () => {
    describe('registration', () => {
      ...
      it('admin is able to update registration contract address', async () => {
        await expect(this.pebbleLoot.connect(admin).updateRegistration(this.registration.address)).to.emit(this.pebbleLoot, 'RegistrationUpdated').withArgs(this.registration.address);
      });
      it('should revert if non admin tries to update', async () => {
        await expect(this.pebbleLoot.connect(minter).updateRegistration(this.registration.address)).to.be.revertedWith('Ownable: caller is not the owner');
      });
    });
    describe('minting fee', () => {
      const newMintingFee = 300;
      it('admin is able to update minting fee', async () => {
        await expect(this.pebbleLoot.connect(admin).updateMintingFee(newMintingFee)).to.emit(this.pebbleLoot, 'MintingFeeUpdated').withArgs(newMintingFee);
      ...
      it('should revert if non admin tries to update minting fee', async () => {
        expect(this.pebbleLoot.connect(minter).updateMintingFee(newMintingFee)).to.be.revertedWith('Ownable: caller is not the owner');
      });
    });
    describe('fee receiver', () => {
      it('should allow admin update fee receiver address', async () => {
        await expect(this.pebbleLoot.connect(admin).updateFeeReceipient(admin.address)).to.emit(this.pebbleLoot, 'FeeReceipientUpdated').withArgs(admin.address);

        expect(await this.pebbleLoot.feeReceipient()).to.be.equal(admin.address);
      });
      it('should revert if non admin tries to update receiver address', async () => {
        await expect(this.pebbleLoot.connect(minter).updateFeeReceipient(minter.address)).to.be.revertedWith('Ownable: caller is not the owner');
      });
    });
  });
  ...
  describe('minting', () => {
    it('should revert if provided IMEI is incorrect', async () => {
      ...
      expect(this.pebbleLoot.connect(minter).claim(idBelowRange)).to.be.revertedWith('Claim: Token ID is invalid');

      expect(this.pebbleLoot.connect(minter).claim(idAboveRange)).to.be.revertedWith('Claim: Token ID is invalid');
    });
    it('should revert if minter doesnt owns the device', async () => {
      const imei = 100000000000000;
      expect(this.pebbleLoot.connect(minter).claim(imei)).to.be.revertedWith('You should own the device to mint this loot');
    });
    it.skip('device owner should be able to mint nft', async () => {
      await expect(this.pebbleLoot.connect(deviceOwner1).claim(imei1)).to.emit(this.pebbleLoot, 'TokenMinted').withArgs(deviceOwner1.address, imei1);
    });
  });
